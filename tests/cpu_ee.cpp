...
TEST_CASE("Data Processing - MOV immediate", "[cpu_ee]") {
  ...
  REQUIRE(cpu.getRegister(15) ==
          initial_pc); // PC should not change for non-branch instructions
...
TEST_CASE("Data Processing - CMP sets flags but doesn't write result",
          "[cpu_ee]") {
  Memory mem;
  TestableExecutionEngine cpu(&mem, 0x1000);
  ...
  REQUIRE(result.wroteMemory == false);
  REQUIRE(result.wroteCPSR ==
          true); // CMP always updates flags (implicit S bit)
}
...
TEST_CASE("Branch - B (unconditional branch)", "[cpu_ee]") {
  ...
  REQUIRE(cpu.getRegister(15) == initial_pc); // PC writes should be tracked via
                                              // getNextPC(), not registers[15]
  // B writes to PC (R15)
  REQUIRE(result.wroteRegister == true);
...
TEST_CASE("Conditional Execution - NE (not equal), condition false",
          "[cpu_ee]") {
  Memory mem;
  TestableExecutionEngine cpu(&mem, 0x1000);
...
TEST_CASE("Block Data Transfer - Complete PUSH/POP sequence", "[cpu_ee]") {
  ...
  uint32_t stacked_r0 = mem.readByte(sp_after_push1) |
                        (mem.readByte(sp_after_push1 + 1) << 8) |
                        (mem.readByte(sp_after_push1 + 2) << 16) |
                        (mem.readByte(sp_after_push1 + 3) << 24);
  uint32_t stacked_r1 = mem.readByte(sp_after_push1 + 4) |
                        (mem.readByte(sp_after_push1 + 5) << 8) |
                        (mem.readByte(sp_after_push1 + 6) << 16) |
                        (mem.readByte(sp_after_push1 + 7) << 24);
  uint32_t stacked_r2 = mem.readByte(sp_after_push1 + 8) |
                        (mem.readByte(sp_after_push1 + 9) << 8) |
                        (mem.readByte(sp_after_push1 + 10) << 16) |
                        (mem.readByte(sp_after_push1 + 11) << 24);
  uint32_t stacked_r3 = mem.readByte(sp_after_push1 + 12) |
                        (mem.readByte(sp_after_push1 + 13) << 8) |
                        (mem.readByte(sp_after_push1 + 14) << 16) |
                        (mem.readByte(sp_after_push1 + 15) << 24);
