...
TEST_CASE("CPU step returns empty result when halted", "[cpu]") {
    ...
    // Note: halted_pc is in an unmapped memory region, so CPU should detect halt before trying to fetch
    ExecutionResult result = cpu.step();
...
TEST_CASE("CPU getRegister returns correct values", "[cpu]") {
    ...
    REQUIRE(cpu.getRegister(14) == halted_pc); // 14 starts at the halted PC value by default
