/**
    ...
    Headers for the CPU subsystem of the AArch32 emulator.
     This includes:
        - a basic CPU Execution Engine class that provides methods for fetching,
   decoding, and executing instructions. It also includes registers and a
   program counter (PC) to keep track of the current instruction being executed.
 */
...
// Constants
const uint32_t halted_pc =
    0x3fff0000; // A special PC value that indicates the CPU is halted. This is
                // not a valid instruction address.  Initially stored in R14.

// Forward declaration
class Memory;

/**
    Structure to hold information about what an instruction wrote during
   execution.
*/
struct ExecutionResult {
  bool wroteRegister; // True if a register was written
  std::set<int>
      registersWritten;   // Set of register numbers that were written (0-15)
  ...
  ExecutionResult()
      : wroteRegister(false), wroteCPSR(false), wroteMemory(false),
        memoryAddress(0), memorySize(0) {}
};

/**
    This class represents the execution engine of the CPU, responsible for
   executing instruction and managing the CPU state (registers, program counter,
   etc.). It interacts with the memory subsystem to read/write data. The decode
...
protected:
  ...
  bool wrotePC;   // Flag to indicate if the instruction wrote to the PC (for
                  // branching)
  uint32_t newPC; // Temporary variable to hold the new PC value if the
...
private:
  ...
  // Helper methods
  uint32_t applyShift(uint32_t value, uint32_t shift_type,
                      uint32_t shift_amount, bool &carry_out);
