...
CPU_ExecutionEngine::CPU_ExecutionEngine(Memory *mem, uint32_t initial_pc)
  ...
  // Set the link register to the halted PC value by default (this is just a
  // convention for our emulator)
...
bool CPU_ExecutionEngine::shouldExecuteInstruction(AArch32Instruction instr) {
  ...
  case 0x2:
    return C; // CS/HS: Carry set / Unsigned higher or same
  case 0x3:
    return !C; // CC/LO: Carry clear / Unsigned lower
  case 0x4:
  ...
  case 0x7:
    return !V; // VC: No overflow
  case 0x8:
    return C && !Z; // HI: Unsigned higher
  case 0x9:
    return !C || Z; // LS: Unsigned lower or same
  case 0xA:
    return N == V; // GE: Signed greater than or equal
  case 0xB:
    return N != V; // LT: Signed less than
  case 0xC:
    return !Z && (N == V); // GT: Signed greater than
  case 0xD:
    return Z || (N != V); // LE: Signed less than or equal
  case 0xE:
    return true; // AL: Always execute
  default:
    return false; // NV and undefined conditions do not execute
  }
}
...
CPU_ExecutionEngine::executeInstruction(AArch32Instruction instr) {
  ...
  if (instr.common.kind == 0b00) {
    // Check for multiply instruction (special encoding within data processing
    // space)
    if (instr.mul.fixed000000 == 0b000000 && instr.mul.fixed1001 == 0b1001) {
      result = executeMultiply(instr);
    }
    // Check for branch and exchange (special encoding within data processing
    // space) BX encoding: cccc 0001 0010 1111 1111 1111 0001 Rm Check bits 27-4
    // (ignoring condition and Rm)
    else if (instr.common.kind == 0b00 && instr.bx.op == 0b010 &&
    ...
    if (instr.div.fixed110 == 0b110 && instr.div.fixed1111 == 0b1111 &&
...
CPU_ExecutionEngine::executeDataProcessing(AArch32Instruction instr) {
  ...
  if (instr.data_imm.i == 1) {
    ...
    operand2 = applyShift(reg_value, instr.data_reg.shift_type,
                          instr.data_reg.shift_imm, carry_out);
  }
  ...
  case 0x6: // SBC
    ...
    carry_out =
        (operand1 >= operand2) && ((operand1 - operand2) >= (1 - carry));
    overflow = ((operand1 ^ operand2) & (operand1 ^ result)) >> 31;
    break;
  case 0x7: // RSC
    result = operand2 - operand1 - (1 - carry);
    carry_out =
        (operand2 >= operand1) && ((operand2 - operand1) >= (1 - carry));
    overflow = ((operand2 ^ operand1) & (operand2 ^ result)) >> 31;
    break;
  case 0x8: // TST
    result = operand1 & operand2;
  ...
  default:
    throw std::runtime_error(
        std::format("Unknown data processing opcode: {}", opcode));
  }
...
CPU_ExecutionEngine::executeLoadStore(AArch32Instruction instr) {
  ...
  if (is_immediate) {
    ...
    uint32_t reg_value = (rm == 15) ? (registers[15] + 8) : registers[rm];
    bool carry_out;
    offset = applyShift(reg_value, instr.ls_reg.shift_type,
                        instr.ls_reg.shift_imm, carry_out);
  }
