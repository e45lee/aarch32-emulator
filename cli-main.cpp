 ...
 */
bool loadBinaryFile(MemoryMappedIO* memory, const std::string& filename, uint32_t load_address = 0x00000000) {
...
void dumpRegisterState(CPU* cpu) {
    ...
    for (int i = 0; i < 13; i += 4) {
        ...
        for (int j = 0; j < 4 && (i + j) < 13; j++) {
            int reg = i + j;
            std::cerr << "R" << std::setw(2) << std::left << reg << ": 0x"
    ...
    std::cerr << "  CPSR: 0x" << std::hex << std::setw(8) << std::setfill('0') << cpsr
              << "  [" << (N ? "N" : "-") << (Z ? "Z" : "-")
              << (C ? "C" : "-") << (V ? "V" : "-") << "]" << std::endl;
...
void printUsage(const char* program_name) {
    std::cerr << "Usage: " << program_name << " <binary_file> [--r0 VALUE] [--r1 VALUE] ... [--r12 VALUE]" << std::endl;
    std::cerr << std::endl;
    std::cerr << "Options:" << std::endl;
    std::cerr << "  --r0 to --r12   Set initial register values (decimal or hex with 0x prefix)" << std::endl;
    std::cerr << std::endl;
    std::cerr << "MMIO Addresses:" << std::endl;
    std::cerr << "  0x9000000  Console output (write a byte to print to stdout)" << std::endl;
    std::cerr << "  0x9000004  Console input (read a byte from stdin)" << std::endl;
    std::cerr << "  0x9000008  Console status (bit 0: has input, bit 1: can output)" << std::endl;
    std::cerr << std::endl;
    std::cerr << "Examples:" << std::endl;
    std::cerr << "  " << program_name << " program.bin" << std::endl;
    std::cerr << "  " << program_name << " program.bin --r0 42 --r1 0xFF" << std::endl;
    std::cerr << "  echo \"Hello\" | " << program_name << " echo.bin" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (!loadBinaryFile(memory.get(), binary_filename, 0x00000000)) {
        std::cerr << "Error: Failed to load binary file: " << binary_filename << std::endl;
        return 1;
    }
    ...
    try {
        ...
        if (instruction_count >= MAX_INSTRUCTIONS) {
            std::cerr << std::endl;
            std::cerr << "Error: Maximum instruction count reached (" << MAX_INSTRUCTIONS << ")" << std::endl;
        ...
        std::cerr << "Executed " << std::dec << instruction_count << " instructions" << std::endl;
        dumpRegisterState(cpu.get());
        return 1;
    }
}
