...
Memory::Memory(uint32_t lower_end, uint32_t upper_start)
    : lower_end(lower_end), upper_start(upper_start) {
  // Initialize the lower and upper memory regions based on the provided
  // parameters
...
uint8_t Memory::readByte(uint32_t address) {
  if (address < lower_memory.size()) {
    ...
    throw std::out_of_range(
        std::format("Memory read out of bounds: 0x{:08X}, lower_end=0x{:08X}, "
                    "upper_start=0x{:08X}",
                    address, lower_end, upper_start));
  }
}
...
void Memory::writeByte(uint32_t address, uint8_t value) {
  if (address < lower_memory.size()) {
    ...
    // Address is out of bounds
    throw std::out_of_range(
        std::format("Memory write out of bounds: 0x{:08X}, lower_end=0x{:08X}, "
                    "upper_start=0x{:08X}",
                    address, lower_end, upper_start));
  }
}
