...
std::string formatRegister(const std::string &name, uint32_t value) {
  ...
  oss << name << ": 0x" << std::hex << std::setw(8) << std::setfill('0')
      << value;
  return oss.str();
}

std::vector<std::string> getDisassembly(EmulatorState &state, int lines_before,
                                        int lines_after) {
...
std::vector<std::string> getRegisterView(EmulatorState &state) {
  ...
  cpsr_oss << "CPSR: 0x" << std::hex << std::setw(8) << std::setfill('0')
           << cpsr;
...
void setupConsoleIO(EmulatorState &state) {
  ...
  state.memory->setWriteHandlerB(CONSOLE_OUT_ADDR, [&state](uint32_t addr,
                                                            uint8_t value) {
    if (value == '\n') {
    ...
    while (state.console_output.size() >
           static_cast<size_t>(state.max_console_lines)) {
      state.console_output.pop_front();
    }
  });

  // Console input handler
  state.memory->setReadHandlerW(CONSOLE_IN_ADDR,
                                [&state](uint32_t addr) -> uint32_t {
  ...
  state.memory->setReadHandlerB(CONSOLE_STATUS_ADDR,
                                [&state](uint32_t addr) -> uint8_t {
                                  uint8_t status = 0;
                                  return status;
                                });
}
...
void loadTestProgram(EmulatorState &state) {
  uint8_t data[] = {
      0x24, 0x10, 0x8f, 0xe2, 0x1c, 0x20, 0x9f, 0xe5, 0x00, 0x00, 0xd1, 0xe5,
      0x00, 0x00, 0x50, 0xe3, 0x02, 0x00, 0x00, 0x0a, 0x00, 0x00, 0xc2, 0xe5,
      0x01, 0x10, 0x81, 0xe2, 0xf9, 0xff, 0xff, 0xea, 0x1e, 0xff, 0x2f, 0xe1,
      0x04, 0x00, 0x00, 0xea, 0x00, 0x00, 0x00, 0x09, 0x48, 0x65, 0x6c, 0x6c,
      0x6f, 0x2c, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64, 0x0a, 0x00, 0x00, 0x00,
  };
...
std::vector<std::string> getMemoryView(EmulatorState &state, uint32_t address,
                                       int bytes_before, int bytes_after) {
  std::vector<std::string> result;
...
bool loadBinaryFile(EmulatorState &state, const std::string &filename,
  ...
  for (size_t i = 0; i < buffer.size(); i++) {
    try {
      ...
      std::cerr << "Error: Failed to write to memory at address 0x" << std::hex
                << (load_address + i) << ": " << e.what() << std::endl;
      return false;
    }
  }
