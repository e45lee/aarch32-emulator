...
Component createUIRenderer(EmulatorState &state) {
  ...
  auto memory_addr_wrapped =
      CatchEvent(memory_addr_input, [&state](Event event) {
        if (event == Event::Return) {
          try {
            // Parse the hex address
            state.memory_view_address =
                std::stoul(state.memory_address_input, nullptr, 16);
  ...
  auto input_container = Container::Tab(
      {console_input_wrapped, memory_addr_wrapped}, &state.current_tab);

  // Create a renderer that displays all panes
  auto renderer = Renderer(input_container, [&state, console_input_wrapped,
                                             memory_addr_wrapped] {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      reg_elements.push_back(
          hbox({left_reg | size(WIDTH, EQUAL, 18), text(" | "), right_reg}));
    }
    ...
    auto reg_pane =
        window(text("Registers"), vbox(std::move(reg_elements)) | frame);
    ...
    auto stack_pane = window(text("Stack"), vbox(std::move(stack_elements)) |
    ...
    auto console_input_box =
        hbox({text("Input: ") | bold, console_input_wrapped->Render()});

    // Build memory view pane
    auto memory_view =
        getMemoryView(state, state.memory_view_address, 128, 128);
    ...
    auto memory_pane = window(text("Memory"), vbox(std::move(memory_elements)) |
                                                  vscroll_indicator | frame);

    // Build memory address input area
    auto memory_addr_box =
        hbox({text("Address: ") | bold, memory_addr_wrapped->Render() | border,
              text("  ") | dim,
              text("(Press Enter to jump) | PgUp/PgDn: ±256 bytes | Up/Down: "
                   "±16 bytes") |
                  dim});

    // Tab bar
    auto tab_bar =
        hbox({text(state.current_tab == 0 ? " [CPU View] " : "  CPU View  ") |
                  (state.current_tab == 0 ? bgcolor(Color::Blue)
                                          : bgcolor(Color::GrayDark)) |
                  bold,
              text(" | "),
              text(state.current_tab == 1 ? " [Memory View] "
                                          : "  Memory View  ") |
                  (state.current_tab == 1 ? bgcolor(Color::Blue)
                                          : bgcolor(Color::GrayDark)) |
                  bold,
              text(" ") | flex, text("F1: CPU | F2: Memory") | dim}) |
        border;
    ...
    if (state.current_tab == 0) {
      ...
      auto top_row =
          hbox({vbox({reg_pane | size(HEIGHT, LESS_THAN, 20) |
      ...
      auto bottom_area = vbox({separator(), console_input_box});

      main_content = vbox({top_row | flex, separator(), bottom_area});
    } else {
      // Memory View
      main_content = vbox({memory_pane | flex, separator(), memory_addr_box});
    }
    ...
    auto status_bar =
        hbox(
            {text(state.running ? " RUNNING " : " STOPPED ") |
                 (state.running ? bgcolor(Color::Green) : bgcolor(Color::Red)) |
                 bold,
             separator(), text(" " + state.status_message + " ") | flex,
             separator(),
             text(" F5: Run/Stop | F6: Step | F7: Reset | F10: EOF | F12: "
                  "Quit ") |
                 dim}) |
        border;
    ...
    // Combine everything
    return vbox(
        {tab_bar, separator(), main_content | flex, separator(), status_bar});
  });
 ...
 */
Component createEventHandler(Component base, EmulatorState &state,
      ...
      if (!state.loaded_filename.empty()) {
        if (loadBinaryFile(state, state.loaded_filename, 0x00000000)) {
          state.status_message =
              "CPU reset - reloaded: " + state.loaded_filename;
        } else {
