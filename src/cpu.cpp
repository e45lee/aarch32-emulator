...
ExecutionResult CPU::step() {
  ...
  if (pc == halted_pc) {
    // If PC is at the halted address, we consider the CPU to be halted and do
    // not execute further instructions.
...
ExecutionResult CPU::stepUntilPC(uint32_t target_pc) {
  ...
  do {
    result = step();
  } while (execution_engine.getRegister(15) != target_pc &&
           execution_engine.getRegister(15) != halted_pc);
  return result;
}
...
uint32_t CPU::getCPSR() const { return execution_engine.getCPSR(); }
